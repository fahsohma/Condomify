...
function NotificarEncomenda() {
  ...
  const [qtdeItens, setQtdeItens] = useState(1); // Estado para a quantidade de itens
  ...
  const notificarMorador = () => {
    ...
    alert(`Morador ${moradorSelecionado.nome} notificado com ${qtdeItens} itens`);

    // Limpa os campos após a notificação
    setApartamento('');
    setMoradores([]);
    setApartamentoExistente(false);
    setMoradorSelecionado(null);
    setQtdeItens(1); // Reseta a quantidade de itens para 1
        ...
        <form onSubmit={(e) => e.preventDefault()}>
          ...
          {apartamentoExistente && (
            <div>
              ...
              {/* Campo Qtde Itens */}
              <div className="campo-qtde-itens">
                <label>Qtde Itens:</label>
                <input
                  type="number"
                  value={qtdeItens}
                  onChange={(e) => setQtdeItens(Math.max(1, e.target.value))} // Garante que o valor mínimo seja 1
                  min="1"
                  required
                />
              </div>
